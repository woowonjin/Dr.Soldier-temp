...
def notification(request):
    ...
    if noti_type_temp == "like":
        ...
        noti = noti_models.Notification.objects.create(
            user=user, post=post, noti_type=noti_type)
...
def get_notifications(request):
    uid = request.GET.get("user")
    user = user_models.User.objects.get(uid=uid)
    notifications = noti_models.Notification.objects.filter(
        post__host=user).order_by("-created")
    noti_full = []
...
def get_notifications_num(request):
    uid = request.GET.get("user")
    user = user_models.User.objects.get(uid=uid)
    notis_num = noti_models.Notification.objects.filter(
        post__host=user).filter(is_read=False).count()
